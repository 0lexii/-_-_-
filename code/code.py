...
def create_main_window():
    ctk.set_appearance_mode("Light")  # Темна/світла тема базуючись на системних налаштуваннях
    ctk.set_default_color_theme("blue")  # Тема кольорів

    root = ctk.CTk()  # Використовуємо CTk замість Tk
    ...
    # Вкладки для входу та реєстрації
...
# Функція для створення інтерфейсу входу
...
# Функція для створення інтерфейсу реєстрації
...
# Функція для відкриття домашнього вікна
...
# Функція для створення інтерфейсу теорії
...
def display_content(topic, content, text_widget):
    ...
    if topic == "Тема 1. Das Präsens":
        # Load the image
        image = PhotoImage(file="prasens.png")

        # Insert the image into the Text widget at the end of the text
    ...
    if topic == "Тема 2. Das Präteritum":
        # Load the image
        image = PhotoImage(file="prateritum.png")
    ...
    # Функція для створення інтерфейсу тестування
...
def create_settings_ui(parent):
    ...
    def toggle_theme():
        ...
        if current_theme == "світла":
            # Змініть кольори для світлої теми
            root.configure(bg="white")
            # Інші віджети можуть також мати більше світлих кольорів
        elif current_theme == "темна":
            # Змініть кольори для темної теми
            root.configure(bg="black")
            # Інші віджети можуть також мати більше темних кольорів

    theme_var = ctk.BooleanVar(value=True)  # Початкова тема - світла
    ...
    def logout():
        if messagebox.askyesno("Підтвердження", "Ви впевнені, що хочете вийти з акаунту?"):
            parent.destroy()  # Закрити головне вікно
...
def open_test_window(topic=""):
    ...
    # Створення прокручуваної області для питань
    ...
    # Завантаження питань для вибраної теми
